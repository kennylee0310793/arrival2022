...
{
    public static AudioClip FromMp3Data_L(byte[] data)
    {
    ...
    public static AudioClip FromMp3Data_R(byte[] data)
    {
        // Load the data into a stream
        MemoryStream mp3stream = new MemoryStream(data);
        // Convert the data in the stream to WAV format
        ...
        {
            audioClip = AudioClip.Create("Audio File Name", wav.SampleCount, 1, wav.Frequency, false);
            audioClip.SetData(wav.RightChannel, 0);
        }
        else
        {
            audioClip = AudioClip.Create("Audio File Name", wav.SampleCount, 1, wav.Frequency, false);
    ...
    //wav only support standard 1411kbps format
    public static AudioClip FromWav_L(byte[] data)
    {
    ...
    public static AudioClip FromWav_R(byte[] data)
    {
        WAV wav = new WAV(data);
